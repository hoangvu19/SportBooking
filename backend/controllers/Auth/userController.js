...
class UserController {
    static async getUserProfile(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                message: req.t('auth.profile_fetch_error')
            });
        }
    }
        ...
        try {
            ...
            if (parseInt(userId) !== currentUserId) {
                return res.status(403).json({
                    success: false,
                    message: req.t('auth.unauthorized_profile_update')
                });
            }
            ...
            res.status(500).json({
                success: false,
                message: error.message || req.t('auth.profile_update_error')
            });
        }
    }
    static async uploadAvatar(req, res) {
        try {
            ...
            if (parseInt(userId) !== currentUserId) {
                return res.status(403).json({ success: false, message: req.t('auth.unauthorized_avatar_update') });
            }

            if (!req.file) {
                return res.status(400).json({ success: false, message: req.t('auth.no_file_sent') });
            }
            ...
            res.status(500).json({ success: false, message: req.t('auth.avatar_upload_error') });
        }
    }
    static async getUserPosts(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                message: req.t('auth.posts_fetch_error')
            });
        }
    }
        static async getUserLikedPosts(req, res) {
            try {
                ...
                res.status(500).json({ success: false, message: req.t('auth.liked_posts_fetch_error') });
            }
        }
    ...
    static async getFollowers(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                message: req.t('auth.followers_fetch_error')
            });
        }
    }
    ...
    static async getFollowing(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                message: req.t('auth.following_fetch_error')
            });
        }
    }
