...
const StoryViewDAL = require('../../DAL/Social/StoryViewDAL');
...
async function createStory(req, res) {
  ...
  const formatted = story.toFrontendFormat();
    ...
    // Emit realtime event so other connected clients can update immediately
    try {
      const realtimeEmitter = require('../../lib/realtimeEmitter');
      // broadcast to all clients in the /realtime namespace
      realtimeEmitter.emitEvent('story:created', null, formatted);
    } catch (e) {
      console.debug('Could not emit realtime story:created', e && e.message);
    }
...
async function deleteStory(req, res) {
  try {
    ...
    // Emit realtime event so all connected clients can remove the deleted story
    try {
      const realtimeEmitter = require('../../lib/realtimeEmitter');
      realtimeEmitter.emitEvent('story:deleted', null, { 
        story_id: idCheck.value,
        _id: idCheck.value 
      });
      console.log(`✅ Emitted story:deleted event for story ${idCheck.value}`);
    } catch (e) {
      console.debug('Could not emit realtime story:deleted', e && e.message);
    }
