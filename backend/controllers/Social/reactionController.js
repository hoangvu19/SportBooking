...
class ReactionController {
  ...
  static async createOrUpdateReaction(req, res) {
    try {
      ...
      // Emit realtime event for reaction changes so clients update counts
      try {
        if (result && (result.action === 'created' || result.action === 'removed' || result.action === 'updated')) {
          const emitter = require('../../lib/realtimeEmitter');
          // Debug: log emit details (temporary)
          try {
            const emitPreview = (result && result.data) ? result.data : result;
            console.log('🔔 Emitting reaction event', { action: result.action, postId: idCheck.value, accountId, reaction: emitPreview });
          } catch (logErr) {
            console.debug('🔔 Emitting reaction event (could not serialize preview)', logErr);
          }

          // Include postId and accountId so clients can map the reaction to the correct post
          emitter.emitEvent('reaction:created', null, { reaction: result, postId: idCheck.value, accountId });
          console.log('✅ Emitted reaction:created for postId', idCheck.value);
        }
      } catch (e) { console.error('❌ Error emitting reaction event', e); }
