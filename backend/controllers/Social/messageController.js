...
const MessageController = {
  ...
  async sendMessage(req, res) {
    try {
      ...
      // Emit realtime event to sender and receiver rooms so other clients see the new message
      try {
        const emitter = require('../../lib/realtimeEmitter');
        // rooms are 'user:{userId}'
        const senderRoom = `user:${String(senderId)}`;
        const receiverRoom = `user:${String(receiverId)}`;
        const payload = { message: formatted };
        // emit to receiver so they receive the message in real-time
        emitter.emitEvent('message:received', receiverRoom, payload);
        // also notify sender room so other tabs/devices of sender get it
        emitter.emitEvent('message:received', senderRoom, payload);
      } catch (e) {
        console.debug('Realtime emit failed for message', e && e.message);
      }
    } catch (error) {
  ...
  async markAsRead(req, res) {
    try {
      ...
      // Notify the sender (and sender's other devices) that their messages were read
      try {
        const emitter = require('../../lib/realtimeEmitter');
        const senderRoom = `user:${String(sId)}`;
        const payload = { senderId: sId, receiverId, readAt: new Date().toISOString() };
        emitter.emitEvent('message:read', senderRoom, payload);
      } catch (e) {
        console.debug('Realtime emit failed for message:read', e && e.message);
      }
    } catch (error) {
