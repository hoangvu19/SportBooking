...
async function setRating(req, res) {
  try {
    ...
    if (result.success) {
      // Fetch fresh stats to include in realtime payload
      try {
        const stats = await RatingDAL.getRatingStats(targetType, parseInt(targetId));
        // Fire realtime update so clients can refresh rating displays
        try {
          const emitter = require('../../lib/realtimeEmitter');
          emitter.emitEvent('rating:updated', null, {
            targetType,
            targetId: parseInt(targetId),
            action: result.isUpdate ? 'updated' : 'created',
            rating: result.data,
            stats: stats && stats.data ? stats.data : null
          });
        } catch (e) { /* ignore realtime emit failures */ }
