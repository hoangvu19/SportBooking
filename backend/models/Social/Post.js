...
class Post {
	...
	toFrontendFormat() {
		...
		// Normalize Images array into image_urls and media_urls
		const allMedia = Array.isArray(this.Images) ? this.Images.map(mapImageUrl).filter(Boolean) : [];
		const isVideoUrl = (u) => {
			if (!u || typeof u !== 'string') return false;
			if (u.startsWith('data:video')) return true;
			return !!u.match(/\.(mp4|webm|ogg|mov)(\?|$)/i);
		};
		const image_urls = allMedia.filter(u => !isVideoUrl(u));
		const media_urls = allMedia.filter(u => isVideoUrl(u));
		...
		return {
			...
			// Split media into image_urls and media_urls (videos)
			image_urls,
			media_urls,
			post_type: postType,
