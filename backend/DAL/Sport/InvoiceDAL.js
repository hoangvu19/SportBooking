...
class InvoiceDAL {
  static async createInvoice(invoiceData) {
    // Use a simple pooled request instead of a separate transaction here.
    // The booking creation already commits its own transaction; creating
    // a new transaction on the same pool can lead to unexpected aborts
    // (EABORT). A single INSERT is atomic enough without an explicit tx.
    try {
  ...
  .input('Status', sql.NVarChar(50), 'Unpaid')
      ...
      console.error('InvoiceDAL.createInvoice error:', error);
      throw error;
    }
  }
