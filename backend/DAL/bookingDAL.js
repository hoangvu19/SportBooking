  ...
  static async create(bookingData) {
    ...
    try {
      ...
      const conflictCheck = await transaction.request()
        ...
        .input('StartTime', sql.DateTime, bookingData.StartTime)
        .input('EndTime', sql.DateTime, bookingData.EndTime)
        .query(`
      ...
      const result = await transaction.request()
        ...
        .input('StartTime', sql.DateTime, bookingData.StartTime)
        .input('EndTime', sql.DateTime, bookingData.EndTime)
        .input('Status', sql.VarChar, bookingData.Status || 'Pending')
        .input('Deposit', sql.Decimal(10, 2), bookingData.Deposit || 0)
        .query(`
          INSERT INTO Booking (FieldID, CustomerID, StartTime, EndTime, Status, Deposit, CreatedDate)
          OUTPUT INSERTED.BookingID
          VALUES (@FieldID, @CustomerID, @StartTime, @EndTime, @Status, @Deposit, GETDATE())
        `);
      
      await transaction.commit();
      
      const newBookingId = result.recordset[0].BookingID;
      return await BookingDAL.getById(newBookingId);
    } catch (error) {
      await transaction.rollback();
      console.error('BookingDAL.create error:', error);
      throw error;
    }
  }
