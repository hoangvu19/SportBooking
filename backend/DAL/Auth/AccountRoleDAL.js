...
class AccountRoleDAL {
  static async assignRole(accountId, roleId) {
    ...
    try {
      ...
      if (existing.recordset.length > 0) {
        ...
        // Insert using minimal columns to support schemas without AssignedDate
  ...
  static async getAccountRoles(accountId) {
    ...
    const result = await pool.request()
      .input('AccountID', sql.Int, accountId)
      .query(`
          SELECT ar.*, r.RoleName
        FROM AccountRole ar
        INNER JOIN Role r ON ar.RoleID = r.RoleID
        WHERE ar.AccountID = @AccountID
          ORDER BY ar.AccountRoleID DESC
      `);
  ...
  static async getAllAssignments() {
    ...
    const result = await pool.request()
      .query(`
          SELECT ar.*, a.Username, a.FullName, r.RoleName
        FROM AccountRole ar
        INNER JOIN Account a ON ar.AccountID = a.AccountID
        INNER JOIN Role r ON ar.RoleID = r.RoleID
          ORDER BY ar.AccountRoleID DESC
