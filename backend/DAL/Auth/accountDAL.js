...
const AccountRoleDAL = require("./AccountRoleDAL");
...
class AccountDAL {
  ...
  static async getAll() {
    try {
      ...
      const accounts = result.recordset.map(row => new Account(row));
      for (const account of accounts) {
        try {
          const roles = await AccountRoleDAL.getAccountRoles(account.AccountID);
          account.Roles = roles || [];
        } catch (roleErr) {
          account.Roles = [];
        }
  ...
  static async getById(id) {
    try {
      ...
      try {
        ...
        // Don't fail the whole request if roles lookup fails; log and continue
        console.warn('AccountDAL.getById: failed to fetch roles', roleErr && (roleErr.message || roleErr));
        account.Roles = [];
      }
  ...
  static async getByUsername(username) {
    try {
      ...
      try {
        ...
        console.warn('AccountDAL.getByUsername: failed to fetch roles', roleErr && (roleErr.message || roleErr));
        account.Roles = [];
      }
  ...
  static async getByEmail(email) {
    try {
      ...
      try {
        ...
        console.warn('AccountDAL.getByEmail: failed to fetch roles', roleErr && (roleErr.message || roleErr));
        account.Roles = [];
      }
  ...
  static async getByIdentifier(identifier) {
    try {
      ...
      try {
        ...
        console.warn('AccountDAL.getByIdentifier: failed to fetch roles', roleErr && (roleErr.message || roleErr));
        account.Roles = [];
      }
  ...
  const req = transaction.request()
    .input("Username", sql.NVarChar, accountData.Username)
    .input("PasswordHash", sql.NVarChar, accountData.PasswordHash)
    .input("FullName", sql.NVarChar, accountData.FullName)
    .input("Email", sql.NVarChar, accountData.Email)
    .input("PhoneNumber", sql.NVarChar, accountData.PhoneNumber || null)
    .input("Address", sql.NVarChar, accountData.Address || null)
    .input("Bio", sql.NVarChar(sql.MAX), accountData.Bio || null)
    .input("Status", sql.NVarChar, accountData.Status || "Active")
    .input("AvatarUrl", sql.NVarChar(sql.MAX), accountData.AvatarUrl || null);

  const result = await req.query(`
      INSERT INTO Account (Username, PasswordHash, FullName, Email, PhoneNumber, Address, Bio, Status, AvatarUrl, CreatedAt, UpdatedAt)
      OUTPUT INSERTED.*
      VALUES (@Username, @PasswordHash, @FullName, @Email, @PhoneNumber, @Address, @Bio, @Status, @AvatarUrl, GETDATE(), GETDATE())
    `);
  ...
  static async update(id, accountData) {
    ...
    try {
      ...
      if (accountData.FullName !== undefined || accountData.fullName !== undefined) {
        fields.push("FullName = @FullName");
        request.input("FullName", sql.NVarChar, accountData.FullName || accountData.fullName);
      }
      
      if (accountData.Email !== undefined || accountData.email !== undefined) {
        fields.push("Email = @Email");
        request.input("Email", sql.NVarChar, accountData.Email || accountData.email);
      }
      ...
      if (accountData.PhoneNumber !== undefined) {
        ...
        request.input("PhoneNumber", sql.NVarChar, accountData.PhoneNumber);
      }
      
      if (accountData.Address !== undefined || accountData.address !== undefined) {
        fields.push("Address = @Address");
        request.input("Address", sql.NVarChar, accountData.Address || accountData.address);
      }

      if (accountData.Gender !== undefined || accountData.gender !== undefined) {
        fields.push("Gender = @Gender");
        request.input("Gender", sql.NVarChar, accountData.Gender || accountData.gender);
      }
  ...
  request.input("Status", sql.NVarChar, accountData.Status);
      ...
      await transaction.commit();
      console.log('Transaction committed successfully for AccountID:', id);
  ...
  static async phoneExists(phoneNumber, excludeId = null) {
    try {
      ...
      let query = "SELECT COUNT(*) as count FROM Account WHERE PhoneNumber = @PhoneNumber AND Status != 'Deleted'";
      ...
      console.error('AccountDAL.phoneExists error:', error);
      throw error;
    }
  }
}
