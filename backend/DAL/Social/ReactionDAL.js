...
class ReactionDAL {
    ...
    try {
      ...
      if (existingResult.recordset.length > 0) {
        ...
        if (existing.ReactionType === reactionData.ReactionType) {
          ...
          await pool.request()
            .input("ReactionID", sql.Int, existing.ReactionID)
            .input("ReactionType", sql.NVarChar(50), reactionData.ReactionType)
            .query(`
              UPDATE Reaction 
  ...
  /**
   * Get user's reactions with post details (for AI recommendations)
   * @param {number} accountId 
   * @returns {Array} User's reactions with sport type info
   */
  static async getUserReactionsWithDetails(accountId) {
    try {
      const pool = await poolPromise;
      ...
      console.error('ReactionDAL.getUserReactionsWithDetails error:', error);
      throw error;
    }
  }
}
