...
class MessageDAL {
  ...
  static async createWithImages(messageData, files = []) {
    ...
    try {
      ...
      // Process files sequentially - convert to base64; support images and videos
      for (const f of files || []) {
        ...
        if (f.mimetype.startsWith('image/')) {
          ...
          // Store image via MediaAsset
          try {
            const MediaAssetDAL = require('./MediaAssetDAL');
            await MediaAssetDAL.createMedia({ targetType: 'Message', targetId: messageId, data: base64Image, mediaType: 'Image', accountId: null });
          } catch (imgInsErr) {
            console.warn('MessageDAL.createWithImages - image insert failed:', imgInsErr && imgInsErr.message ? imgInsErr.message : imgInsErr);
          }

        } else if (f.mimetype.startsWith('video/')) {
          // Videos: impose size check (already limited by multer) but double-check here (50MB)
          ...
          // Read file into buffer and store as data URI (no processing)
          const buff = fs.readFileSync(f.path);
          const dataUri = `data:${f.mimetype};base64,${buff.toString('base64')}`;

          // remove temp file
          try { fs.unlinkSync(f.path); } catch (e) { /* ignore */ }

          // Store video via MediaAsset (mediaType 'Video')
          try {
            const MediaAssetDAL = require('./MediaAssetDAL');
            await MediaAssetDAL.createMedia({ targetType: 'Message', targetId: messageId, data: dataUri, mediaType: 'Video', accountId: null });
          } catch (vidErr) {
            console.warn('MessageDAL.createWithImages - video insert failed:', vidErr && vidErr.message ? vidErr.message : vidErr);
          }
    ...
    try {
      ...
      if (images && images.length > 0) {
        try {
          ...
          await transaction.begin();
          try {
            // Store all images (base64 or URL) into MediaAsset
            const MediaAssetDAL = require('./MediaAssetDAL');
            for (let i = 0; i < Math.min(5, images.length); i++) {
              const img = images[i];
              if (typeof img === 'string' && img.trim()) {
                await MediaAssetDAL.createMedia({ targetType: 'Message', targetId: messageId, data: img, mediaType: 'Image', accountId: null });
              }
            }
  ...
  static async deleteById(messageId, requesterId) {
    try {
      ...
      try {
        // Prefer deleting via MediaAsset table
        try {
          const MediaAssetDAL = require('./MediaAssetDAL');
          await MediaAssetDAL.deleteByTarget('Message', messageId);
        } catch (maErr) {
          // fallback to legacy table delete if MediaAsset not present
          try {
        ...
        console.warn('Error attempting to delete message images:', outerErr && outerErr.message ? outerErr.message : outerErr);
      }
  ...
  static async getById(messageId) {
    try {
      ...
      try {
        const MediaAssetDAL = require('./MediaAssetDAL');
        const rows = await MediaAssetDAL.getByTarget('Message', messageId);
        images = rows.map(r => ({ id: r.MediaID, data: r.Data || r.URL }));
      } catch (imgErr) {
        images = [];
      }
    ...
    try {
      ...
      if (messageIds.length > 0) {
        ...
        const imagesQuery = `SELECT ImageID, MessageID, ImageData FROM MessageImage WHERE MessageID IN (${inClause}) ORDER BY ImageID ASC`;
          ...
          try {
            const MediaAssetDAL = require('./MediaAssetDAL');
            const rows = await MediaAssetDAL.getByTargets('Message', messageIds);
            imagesByMessage = {};
            for (const img of rows) {
              if (!imagesByMessage[img.TargetID]) imagesByMessage[img.TargetID] = [];
              imagesByMessage[img.TargetID].push({ id: img.MediaID, data: img.Data || img.URL });
            }
          } catch (imgFetchErr) {
            imagesByMessage = {};
          }
