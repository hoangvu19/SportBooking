...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, `facility-${uniqueSuffix}-${nameWithoutExt}${ext}`);
  }
});
...
const uploadFacilityMedia = upload.array('images', 8);

const handleUploadError = (req, res, next) => {
  uploadFacilityMedia(req, res, function (err) {
    if (err) {
...
module.exports = {
  uploadFacilityMedia: handleUploadError,
  uploadDir,
};
