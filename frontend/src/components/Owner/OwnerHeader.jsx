...
const SearchWithToggle = ({ onSearch }) => {
  ...
  const handleKey = (e) => {
    if (e.key === 'Enter') {
      ...
      try {
        window.dispatchEvent(new CustomEvent('ownerSearch', { detail: q }));
      } catch (err) {
  ...
  return (
    <div className="relative">
      ...
      {q && (
        <button type="button" onClick={() => { setQ(''); onSearch && onSearch(''); }} className="absolute right-10 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1" title={t('common.clear') || 'Clear'}>
          ✕
        </button>
      )}
...
const OwnerHeader = ({ onSearch, showSearch = true }) => {
  ...
  return (
  <header className="sticky top-0 z-40 flex items-center justify-between p-4 bg-white border-b shadow-sm">
      <div className="flex-1">
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            {showSearch && <SearchWithToggle onSearch={onSearch} />}
          </div>
        </div>
      </div>
            ...
            {avatarOpen && (
              <div ref={avatarRef} className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded shadow-lg z-50">
                      ...
                      user.roles.map((r, i) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              try {
                                // compute a normalized role value to keep activeRole canonical
                ...
                <div className="px-3 py-2 border-t">
                  <button onClick={() => { try { logout(); window.location.replace('/login'); } catch (e) { console.warn(e); } }} className="w-full text-left px-3 py-2 rounded hover:bg-gray-100 text-sm text-red-600">{t('settings.logout') || 'Log out'}</button>
                </div>
              </div>
            )}
          </div>
