...
import { ratingAPI, feedbackAPI } from '../../utils/api';
...
const FeedbackSection = ({ targetType = 'Field', targetId }) => {
  ...
  // Fetch rating stats
  const fetchRatingStats = useCallback(async (bypass = false) => {
    try {
      const result = await ratingAPI.getStats(targetType, targetId, bypass);
      if (result.success) {
  ...
  // Fetch comments
  const fetchComments = useCallback(async (page = 1, bypass = false) => {
    try {
      setLoadingComments(true);
  const result = await feedbackAPI.getByTarget(targetType, targetId, page, 20, bypass);
      if (result.success) {
        if (page === 1) {
  ...
  // Listen for realtime rating updates and patch stats in-place
  useEffect(() => {
    const handler = (e) => {
      try {
        const payload = e && e.detail ? e.detail : e;
        if (!payload) return;
        if (String(payload.targetType) !== String(targetType)) return;
        if (String(payload.targetId) !== String(targetId)) return;
        // Update stats if provided
        if (payload.stats) {
          setRatingStats(payload.stats);
        } else {
          // As a fallback, refetch stats (will usually be fast)
          fetchRatingStats();
        }

        // If payload contains rating by current user, update myRating too
        try {
          const ratingRow = payload.rating || payload.ratingRow || null;
          const currentUserId = user && (user.AccountID || user.userId || user.id);
          if (ratingRow) {
            const authorId = ratingRow.AccountID || ratingRow.accountId || ratingRow.UserID || ratingRow.userId;
            if (currentUserId && String(authorId) === String(currentUserId)) {
              const val = ratingRow.Rating || ratingRow.rating || null;
              if (val) setMyRating(val);
            }
          }
  } catch { /* ignore */ }
      } catch (err) { console.debug('rating:updated handler error', err); }
    };

    window.addEventListener('rating:updated', handler);
    return () => window.removeEventListener('rating:updated', handler);
  }, [targetType, targetId, fetchRatingStats, user]);

  // Listen for realtime comment/feedback creations and insert them into the list
  useEffect(() => {
    const handler = (e) => {
      try {
        const payload = e && e.detail ? e.detail : e;
        if (!payload) return;
        // Ensure payload is for this target
        if (String(payload.targetType) !== String(targetType)) return;
        if (String(payload.targetId) !== String(targetId)) return;
        const newComment = payload.comment || payload.data || null;
        if (!newComment) return;

        setComments((prev) => {
          try {
            // Avoid duplicates: match by known id fields
            const newId = String(newComment.FeedbackID || newComment.FeedbackId || newComment.id || newComment.CommentID || newComment.commentId || '');
            if (!newId) return [newComment, ...prev];
            if (prev.some(c => String(c.FeedbackID || c.FeedbackId || c.id || c.CommentID || c.commentId) === newId)) return prev;
            return [newComment, ...prev];
          } catch {
            return [newComment, ...prev];
          }
        });
      } catch (err) { console.debug('comment:created handler error', err); }
    };
  ...
  const handleRatingClick = async (rating) => {
    ...
    // Validate target info before calling API
    if (!targetType || !targetId) {
      console.error('Attempted to save rating but missing targetType or targetId', { targetType, targetId });
      toast.error(t('feedback.missingTarget','Missing target information'));
      return;
    }
  ...
  // Refresh stats and bypass client GET cache to ensure fresh data
  await fetchRatingStats(true);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    // Validate target info before calling API
    if (!targetType || !targetId) {
      console.error('Attempted to submit feedback but missing targetType or targetId', { targetType, targetId });
      toast.error(t('feedback.missingTarget','Missing target information'));
      return;
    }
  ...
  const result = await feedbackAPI.create({ targetType, targetId, content: commentContent });

      if (result.success) {
        setCommentContent('');
        // Reload comments and bypass client GET cache to ensure fresh data
        await fetchComments(1, true);
        setCommentPage(1);
        toast.success(t('feedback.reviewSubmitted'));
        // Also dispatch a global custom event so other open pages/components can update immediately
        try {
          window.dispatchEvent(new CustomEvent('comment:created', { detail: { targetType, targetId, comment: result.data } }));
  } catch { /* ignore */ }
      } else {
        ...
        {loadingComments && commentPage === 1 ? (
          ...
          <>
            {comments.map((comment) => (
              <div key={comment.FeedbackID} className="flex space-x-3 p-4 bg-gray-50 rounded-lg">
                <img
                  src={comment.AvatarUrl || DEFAULT_AVATAR}
