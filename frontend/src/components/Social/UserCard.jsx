    ...
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
        <div className='text-center'>
            ...
            <p className='mt-4 font-semibold cursor-pointer' onClick={() => navigate(`/profile/${user._id}`)}>
                {user.full_name}
                {isFollowing && <span className='ml-2 inline-block text-xs px-2 py-0.5 rounded bg-green-100 text-green-800 font-medium'>{t('profile.following', 'Following')}</span>}
            </p>
        ...
        <div className='flex items-center justify-center gap-2 mt-4 text-xs text-gray-600'>
            <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
                <MapPin className='w-4 h-4'/> {user.location || user.address || t('user.unknown', 'Unknown')}
            </div>
            <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
                <span>{user.followers_count || user.followers?.length || 0}</span> {t('user.followers', 'Followers')}
            </div>
        </div>
        ...
        <div className='mt-4 flex gap-2'>
            ...
            >
                <UserPlus className='w-4 h-4' /> {isFollowing ? (loading ? t('common.processing', 'Processing...') : t('profile.following', 'Following')) : (loading ? t('common.processing', 'Processing...') : t('profile.follow', 'Follow'))}
            </button>
            {/* Message Button */}
            <button 
