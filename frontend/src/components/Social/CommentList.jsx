...
const CommentList = ({ postId, reloadTrigger, scrollToCommentId = null }) => {
  ...
  // Track known comment keys to avoid duplicates across load + realtime events
  const knownIdsRef = useRef(new Set());

  // Normalize various id fields into a string key
  const normalizeId = useCallback((obj) => {
    if (!obj || typeof obj !== 'object') return null;
    const candidates = [
      'CommentID','CommentId','commentId','commentID','Comment_Id','FeedbackID','FeedbackId','feedbackId','_id','id'
    ];
    for (const k of candidates) {
      if (obj[k]) return String(obj[k]);
      const lower = k.toLowerCase();
      if (obj[lower]) return String(obj[lower]);
    }
    return null;
  }, []);

  const makeKey = useCallback((obj) => {
    const id = normalizeId(obj);
    if (id) return `id:${id}`;
    try {
      const author = obj.user?.AccountID || obj.user?._id || obj.AccountID || obj.userId || obj.UserID || obj.user?.id || '';
      const content = (obj.Content || obj.content || '').trim().slice(0,200);
      const time = obj.createdAt || obj.CreatedAt || obj.CreatedDate || obj.created_at || '';
      return `hash:${String(author)}:${content}:${String(time)}`;
    } catch (e) { void e; }
    return null;
  }, [normalizeId]);

  const isLikelySame = useCallback((a, b) => {
    if (!a || !b) return false;
    const aid = normalizeId(a);
    const bid = normalizeId(b);
    if (aid && bid) return String(aid) === String(bid);
    try {
      const aAuthor = a.user?.AccountID || a.user?._id || a.AccountID || a.userId || a.UserID || a.user?.id || null;
      const bAuthor = b.user?.AccountID || b.user?._id || b.AccountID || b.userId || b.UserID || b.user?.id || null;
      const aContent = (a.Content || a.content || '').trim();
      const bContent = (b.Content || b.content || '').trim();
      const aTime = new Date(a.createdAt || a.CreatedAt || a.CreatedDate || a.created_at || a.createdAtLocal || null).getTime();
      const bTime = new Date(b.createdAt || b.CreatedAt || b.CreatedDate || b.created_at || b.createdAtLocal || null).getTime();
      if (aAuthor && bAuthor && aContent && bContent && String(aAuthor) === String(bAuthor) && aContent === bContent) {
        if (!isNaN(aTime) && !isNaN(bTime)) {
          return Math.abs(aTime - bTime) < 5000; // within 5s -> same
        }
        return true;
      }
    } catch (e) { void e; }
  ...
  const loadComments = useCallback(async (bypass = false) => {
    if (!postId) {
  ...
  const res = await commentAPI.getByPostId(postId, bypass);
  ...
  useEffect(() => {
    // If reloadTrigger changes (e.g., a new comment was created), bypass GET cache
    const bypass = !!reloadTrigger;
    loadComments(bypass);
  ...
  // Listen for realtime comment events and insert them if they belong to this post
  useEffect(() => {
    const handler = (ev) => {
      try {
        ...
        if (!c) return;
        const pid = c.PostID || c.postId || c.PostId || c.postID || c.postId;
        if (!pid) return;
        if (String(pid) !== String(postId)) return;

        const key = makeKey(c);
        if (key && knownIdsRef.current.has(key)) return; // already known

        setComments((prev) => {
          try {
            if (prev.some(x => isLikelySame(x, c))) return prev;
            const next = [...prev, c];
            if (key) knownIdsRef.current.add(key);
            try {
              const el = document.querySelector(`#comment-${normalizeId(c)}`)?.closest('.comments-container') || document.querySelector('.comments-container')?.parentElement;
              const scrollEl = el && el.scrollHeight ? el : null;
              if (scrollEl) {
                setTimeout(() => { try { scrollEl.scrollTop = scrollEl.scrollHeight; } catch (e) { void e; } }, 80);
              }
            } catch (e) { void e; }
            return next;
          } catch (e) { void e; return prev; }
        });
      } catch (e) { void e; }
    };
  ...
  }, [postId, makeKey, normalizeId, isLikelySame]);
  ...
  const renderComment = (c, depth = 0) => {
    ...
    return (
      <div
        ...
        key={commentId}
        className={`mt-2 ${depth > 0 ? 'ml-6' : ''} group transition-all duration-200`}
      >
        <div className="flex gap-3">
          <img
            src={c.user?.profile_picture || c.user?.AvatarUrl || c.ProfilePictureURL || c.user?.avatarUrl || DEFAULT_AVATAR}
            alt=""
            className={`${depth > 0 ? 'w-7 h-7' : 'w-9 h-9'} rounded-full shadow-sm object-cover border-2 border-white`}
            onError={(e) => { e.target.onerror = null; e.target.src = DEFAULT_AVATAR; }}
          />
          <div className="flex-1 space-y-1">
            <div className={`p-3 ${depth > 0 ? 'rounded-lg bg-gray-50' : 'rounded-2xl bg-gray-100'} shadow-sm`}>
              <div className="flex justify-between items-start gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                      className="text-sm text-gray-500 hover:text-gray-700 transition-colors px-2 py-0.5 rounded-full hover:bg-gray-200"
                      onClick={() => setReplyTo(commentId)}
                    >
                      {t('comment.reply')}
                    </button>
                    {/* Report button visible for non-owners on hover */}
                    {!isOwner && (
                      <button
                        className="text-sm text-gray-500 hover:text-red-700 transition-colors px-2 py-0.5 rounded-full hover:bg-gray-200 flex items-center gap-1"
                        onClick={(e) => { e.stopPropagation(); setReportTargetId(commentId); setReportModalOpen(true); }}
                      >
                        <Flag className="w-4 h-4 text-red-500" />
                        <span className="hidden sm:inline">{t('report.report') || 'Báo cáo'}</span>
                      </button>
                    )}
                    {isOwner && (
                ...
                {toShow.map((src, idx) => (
                  <div key={idx} className="relative group rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <button
                        className="px-3 py-1 bg-white/90 rounded-md text-sm font-medium hover:bg-white transition-colors"
