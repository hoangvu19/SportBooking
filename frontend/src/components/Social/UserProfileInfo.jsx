...
import { userAPI, areaAPI } from "../../utils/api";
...
const UserProfileInfo = ({ user, setShowEdit, posts, isOwnProfile, followLoading, onChildFollowChange }) => {
  ...
  // Safely resolve createdAt: prefer nested Account.CreatedAt if present, fall back to top-level createdAt.
  const createdAtRaw = user?.createdAt || user?.Account?.CreatedAt || user?.Account?.createdAt || user?.AccountCreatedAt || user?.created_at || user?.CreatedAt || '';
  // Handle numeric timestamps (ms since epoch) as well as server date strings.
  let createdAtDate = null;
  if (createdAtRaw !== undefined && createdAtRaw !== null && createdAtRaw !== '') {
    try {
      if (typeof createdAtRaw === 'number' || /^[0-9]+$/.test(String(createdAtRaw))) {
        // numeric string or number -> treat as epoch ms
        const asNum = Number(createdAtRaw);
        if (!Number.isNaN(asNum)) createdAtDate = new Date(asNum);
      }
      if (!createdAtDate) {
        createdAtDate = parseServerDatetime(createdAtRaw);
      }
    } catch (err) {
      console.debug('Failed to parse createdAtRaw', createdAtRaw, err);
      createdAtDate = null;
    }
  }
  const hasCreatedAt = createdAtDate && !Number.isNaN(new Date(createdAtDate).getTime());
  // Helpful debug when Joined is missing — will appear in browser console.
  if (!hasCreatedAt) console.debug('UserProfileInfo: no valid createdAt for user prop, raw=', createdAtRaw);
  ...
  useEffect(() => {
    ...
    const loadArea = async () => {
      try {
        if (!user) return;
        const areaId = user.areaId || user.AreaID || user.AreaId || user.area || null;
        if (!areaId) return;
        const res = await areaAPI.getAll();
        if (!mounted) return;
        if (res && res.success && Array.isArray(res.data)) {
          const found = res.data.find(a => String(a.AreaID || a.id) === String(areaId));
          if (found) setAreaName(found.AreaName || found.name || null);
        } else if (Array.isArray(res)) {
          const found = res.find(a => String(a.AreaID || a.id) === String(areaId));
          if (found) setAreaName(found.AreaName || found.name || null);
        }
      } catch (err) {
        console.debug('Could not resolve area name for profile', err);
      }
    };
    loadArea();
          ...
          <div className="flex flex-col items-start w-full">
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {areaName || user.location || t('user.addLocation', 'Add location')}
              </span>
              ...
              <span className="flex items-center gap-1.5">
                ...
                {hasCreatedAt ? (
                  <span className="font-medium">{moment(createdAtDate).format('DD/MM/YYYY')}</span>
                ) : (
