...
export default function useLivestream(user) {
    ...
    try {
      ...
      if (!room) {
        try {
          ...
          const created = await livestreamApi.create({ title: `${u.fullName || u.username || t('livestream.liveDefaultName')}'s stream` });
  ...
  const u = normalizeUser(user || {});
  const fakeSaved = { Content: commentText, CreatedDate: new Date().toISOString().replace('Z', '+07:00'), Author: { FullName: u.fullName || u.username } };
