...
const MyInvitations = () => {
  ...
  createdAt: parseServerDatetime(inv.CreatedDate || inv.createdAt) || new Date(),
  ...
  // Chỉ cho phép admin và owner truy cập, ẩn khỏi user thường
  ...
  <p>Manage invitations to join matches</p>
      ...
      <div className="tabs">
        ...
        >
          ⏳ {t('post.invitations.pending', 'Pending')}
        ...
        >
          ✅ {t('post.invitations.accepted', 'Accepted')}
        ...
        >
          ❌ {t('post.invitations.rejected', 'Rejected')}
      ...
      {error && (
        <div className="error-message">
          ...
          <span>{error}</span>
          <button onClick={fetchInvitations}>{t('post.invitations.retry', 'Retry')}</button>
        </div>
      )}
      ...
      <div className="invitations-list">
        ...
        {!loading && filteredInvitations.length === 0 && (
          <div className="empty-state">
            ...
            <h3>
              {activeTab === 'pending' && t('post.invitations.noPending', 'No invitations yet')}
              {activeTab === 'accepted' && t('post.invitations.noAccepted', 'No accepted invitations')}
              {activeTab === 'rejected' && t('post.invitations.noRejected', 'No rejected invitations')}
            </h3>
            <p>
              {activeTab === 'pending' &&
                t('post.invitations.pendingDescription', 'Invitations to join matches will appear here.')}
              {activeTab === 'accepted' &&
                t('post.invitations.acceptedDescription', 'Accepted invitations will appear here.')}
              {activeTab === 'rejected' &&
                t('post.invitations.rejectedDescription', 'Rejected invitations will appear here.')}
            </p>
          </div>
        )}
            ...
            <div key={invitation.PostID} className="invitation-item">
              ...
              {activeTab === 'accepted' && (
                <div className="status-info accepted">
                  ...
                  <span>{t('post.invitations.youAccepted', 'You have accepted this invitation')}</span>
                </div>
              )}
              ...
              {activeTab === 'rejected' && (
                <div className="status-info rejected">
                  <span className="status-icon">❌</span>
                  <span>{t('post.invitations.youRejected', 'You have rejected this invitation')}</span>
                </div>
              )}
            </div>
          ))}
