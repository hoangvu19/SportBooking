...
const Messages = () => {
    ...
    // Listen for server-pushed incoming messages forwarded by useRealtime
    useEffect(() => {
        const handler = (e) => {
            try {
                const payload = e && e.detail ? e.detail : e;
                const m = payload && payload.message ? payload.message : payload;
                if (!m) return;

                const senderId = m.sender?._id || m.SenderID || m.senderId || m.from || (m.from_user_id ? m.from_user_id : null);
                const receiverId = m.receiver?._id || m.ReceiverID || m.receiverId || m.to || (m.to_user_id ? m.to_user_id : null);
                const curId = currentUser?.AccountID || currentUser?._id || currentUser?.userId || null;
                if (!curId) return;

                // Only process messages involving current user (as sender or receiver)
                if (String(senderId) !== String(curId) && String(receiverId) !== String(curId)) return;

                // Determine the conversation partner id (other user)
                const otherId = String(senderId) === String(curId) ? String(receiverId) : String(senderId);

                const snippet = m.content || m.Content || m.text || '';

                setConnections(prev => {
                    const copy = prev.slice();
                    const idx = copy.findIndex(c => String(c._id) === String(otherId));
                    const now = new Date().toISOString();
                    if (idx === -1) {
                        // new conversation: add minimal entry
                        const newConv = {
                            _id: otherId,
                            full_name: (m.sender && (m.sender.full_name || m.sender.FullName)) || '',
                            username: (m.sender && (m.sender.username || m.sender.Username)) || '',
                            profile_picture: (m.sender && (m.sender.avatar || m.sender.profile_picture)) || DEFAULT_AVATAR,
                            bio: '',
                            lastMessageSnippet: snippet,
                            lastMessageSenderId: senderId,
                            lastMessageDate: now,
                            unreadCount: String(receiverId) === String(curId) ? 1 : 0
                        };
                        return [newConv, ...copy];
                    }
                    ...
                    const conv = { ...copy[idx] };
                    conv.lastMessageSnippet = snippet || conv.lastMessageSnippet;
                    conv.lastMessageSenderId = senderId;
                    conv.lastMessageDate = now;
                    // if current user is the receiver, increment unreadCount
                    if (String(receiverId) === String(curId)) {
            ...
            } catch (err) { console.debug('message:received handler error', err); }
        };
    ...
    // Listen for read receipts from server so the conversation unread counts update
    useEffect(() => {
        const onRead = (e) => {
            try {
                const payload = e && e.detail ? e.detail : e;
                ...
                const curId = currentUser?.AccountID || currentUser?._id || currentUser?.userId || null;
                if (!curId) return;
                // Server emits message:read to the sender's room (senderId). If current user is the sender,
                // then the conversation partner is receiverId and their unread count should be cleared.
                if (String(curId) === String(senderId)) {
                    setConnections(prev => prev.map(c => String(c._id) === String(receiverId) ? { ...c, unreadCount: 0 } : c));
                }
            } catch { /* ignore */ }
        };
        window.addEventListener('message:read', onRead);
        return () => window.removeEventListener('message:read', onRead);
    }, [currentUser]);
                    ...
                    <aside className='w-80 shrink-0 self-start sticky top-24'>
                        <div className='bg-white p-4 rounded shadow flex flex-col'>
                            <div className='flex items-center justify-between mb-3'>
                                ...
                                <button title={t('messages.pendingMessages', 'Tin nhắn chờ')} onClick={() => setShowPending(v => !v)} className={`relative p-2 rounded ${showPending ? 'bg-indigo-100' : 'hover:bg-slate-50'}`}>
