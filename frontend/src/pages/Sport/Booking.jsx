...
import { isAvailableToken } from '../../utils/statusUtils';
import { getVnIsoDate, formatVnDate } from '../../utils/vnTime';
...
export default function Booking() {
					...
					if (resp && resp.success && Array.isArray(resp.data)) {
						...
						if (queryTimeSlot) {
							...
							if (slotIndex >= 0 && slotIndex < resp.data.length) {
								...
								const statusRaw = slot.Status ?? slot.status ?? '';
								if (isAvailableToken(statusRaw)) {
	...
	const isAvailable = isAvailableToken(status);
		...
		<div className="container mx-auto px-4 py-8">
			...
			<div className="bg-white rounded-lg shadow-md p-6 mb-6">
				<div className="flex items-center justify-between mb-4">
					<h2 className="text-lg font-semibold">{t('booking.scheduleFor').replace('{date}', formatVnDate(new Date()))}</h2>
			...
			<div className="bg-white rounded-lg shadow-md p-6">
				...
				<div className="space-y-3">
					...
					<div className="flex items-center justify-between">
						...
						<span className="font-medium">{formatVnDate(new Date())}</span>
					</div>
					<div className="flex items-center justify-between">
